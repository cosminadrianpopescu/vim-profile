...
function! s:DB_getDefault(name)
    ...
    elseif a:name ==# "display_cmd_line"        |return (exists("g:dbext_default_display_cmd_line")?g:dbext_default_display_cmd_line.'':1)
    ...
    elseif a:name ==# "ASE_cmd_options"         |return (exists("g:dbext_default_ASE_cmd_options")?g:dbext_default_ASE_cmd_options.'':'-w 65535')
...
function! dbext#DB_parseQuery(query)
    ...
    elseif matchstr( l:filetype, "perl" ) == "perl"
        ...
        return s:DB_parseHostVariables(query)
    elseif matchstr( l:filetype, "php" ) == "php" || matchstr( l:filetype, "module" ) == "module"
...
""    endif
    ...
    while index > -1
        ...
        let @c = s:DB_get('ignore_variable_regex')
        let @d = inout
...
function! s:DB_parsePHP(query)
    ...
    let leading_var_regexp = '^[\ ]*\$[^ ]\+[ \t\n\r]*='
    let query = substitute(query, leading_var_regexp, '', 'g')
    let drupal_table_names = '{\([^\$][^}]\+\)}'
    let query = substitute(query, drupal_table_names, '\1', 'g')
    let drupal_percentages = '%%'
    let query = substitute(query, drupal_percentages, '%', 'g')
